...
{
    internal class _12_Kth_Smallest_Element
    {
        int[] arr { get; set; }
        int n { get { return arr.Length; } }
        int k;
        public _12_Kth_Smallest_Element()
        {
        ...
        public void Solve()
        {
            ...
            new TimeKLogn().Solver(temp, k);

            arr.CopyTo(temp, 0);
            new TimeN().Solver(temp, k);
        }
    }

    class TimeKLogn
    {
        public void Solver(int[] arr, int k)
        {
            ...
            for (int i = arrLength - 1, j = 0; j < k; i--, j++)
            {
                Swap(arr, i, 0);
                Heapify(arr, i, 0);
            }

            //required value is arr[arr.Length-k]
        }
        ...
        void Heapify(int[] arr, int lastIndex, int index)
        {
            ...
            {
                Swap(arr, actualIndex, index);
                Heapify(arr, lastIndex, actualIndex);
            }
        }

        void Swap(int[] arr, int sf, int st)
        {
            if (arr[sf] != arr[st])
            {
                arr[sf] ^= arr[st];
    ...
    class TimeN
    {
        public void Solver(int[] arr, int k)
        {
            int value = QuickSelect(arr, 0, arr.Length - 1, k);
        }
        ...
        int QuickSelect(int[] arr,int start,int end,int k)
        {
            if (start < end)
            {
                int pivotValue = PivotByMedians(arr, start, end);
                int pivotIndex = FindIndex(arr, start, end, pivotValue);
                Swap(arr, pivotIndex, end);
                int actualPivot = NextPivot(arr, start, end);

                if (actualPivot == k - 1)
                    return arr[k - 1];
                else if (actualPivot > k-1)
                    return QuickSelect(arr, start, actualPivot - 1, k);
                else
                    return QuickSelect(arr, actualPivot + 1, end, k);
            }
            return start == k-1 ? arr[k-1]:-1;
        }

        int NextPivot(int[] arr,int start,int end)
        {
            int lesserCount = start - 1;

            for(int i = start; i < end; i++)
            {
                if (arr[i] < arr[end])
                    Swap(arr,i,++lesserCount);
            }
            Swap(arr, end, ++lesserCount);
            return lesserCount;
        }

        int PivotByMedians(int[] arr,int start,int end)
        {
            ...
            {
                medians[i] = GetMedian(GetElements(arr, 5 * i, 5 * (i + 1) - 1));
            }

            if (count % 5 != 0)
            {
                medians[i] = GetMedian(GetElements(arr, 5 * i, 5 * i + (count % 5) - 1));
            }

            int finalMedian = i == 0 ? arr[0] : PivotByMedians(medians, 0, medians.Length - 1);
        ...
        int[] GetElements(int[] arr,int start,int end)
        {
            int count = end - start+1;
