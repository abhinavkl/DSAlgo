...
{
    internal class _10_Ceiling_In_Sorted_Array
    {
        int[] arr { get; set; }
        int n { get { return arr.Length; } }
        int key;
        public _10_Ceiling_In_Sorted_Array()
        {
        ...
        {
            Sort(arr.Length);
            int[] findCeilsFor = new int[] { 12, 13, 27, 28, 9956, 9957, 9958 };
            foreach (var num in findCeilsFor)
            {
                key = num;
                int ceil = Ceil(0, arr.Length - 1);
                if (ceil != n)
                    Console.WriteLine($"ceil of {key} is {arr[ceil]}");
                else
                    Console.WriteLine($"ceil for {key} not exists");
            }
        }
